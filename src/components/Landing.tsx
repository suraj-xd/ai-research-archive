...
import { PhIcon, Logo, ZigDivider } from "@/components/brand";
import playlistThumbs from "@/data/playlistThumbs.json";

function playlistThumb(url: string): string | null {
	try {
		const list = new URL(url).searchParams.get("list");
		if (!list) return null;
		const vid = (playlistThumbs as Record<string, string>)[list];
		return vid ? `https://i.ytimg.com/vi/${vid}/maxresdefault.jpg` : null;
	} catch {
		return null;
	}
}
	...
	return (
		<section id="landing" className="scroll-mt-20 mb-10">
			<div className="flex justify-between items-center w-full" >
				<div className="relative" style={{ padding: "32px 36px" }}>
					<h1
						style={{
					...
					>
						A structured path from math fundamentals to building
						LLMs from scratch. {totalModules} modules,{" "}
						{totalLessons} lessons, covering everything from
						derivatives to transformers to reinforcement learning.
					</p>
...
function PlaylistCard({ playlist }: { playlist: (typeof playlists)[number] }) {
	const thumb = playlistThumb(playlist.url);
	return (
		<a
			href={playlist.url}
			target="_blank"
		...
		>
			{/* Thumbnail (16:9 cover, falls back to a YouTube-red gradient) */}
			<div
				className="relative w-full overflow-hidden"
				style={{ aspectRatio: "16 / 9", background: "var(--ga-chip)" }}
			>
				{thumb ? (
					<img
						src={thumb}
						...
						onError={(e) => {
							...
							// maxresdefault → hqdefault → mqdefault → hide
					...
					<div
						...
						style={{
							background:
								"linear-gradient(135deg, rgba(255,0,0,0.06) 0%, rgba(0,0,0,0.04) 100%)",
						}}
					>
						<PhIcon
							name="youtube-logo"
		...
		<section id="resources" className="scroll-mt-20 mb-12">
			...
			{(activeTab === "all" || activeTab === "videos") && (
				<div className="mb-8 fade-in">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
						{playlists.map((pl) => (
							<PlaylistCard key={pl.title} playlist={pl} />
						))}
					</div>
				</div>
			)}
